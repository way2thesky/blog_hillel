...
class RegisterFormView(SuccessMessageMixin, FormView):
    template_name = 'blog_hillel/registration/register.html'
...
class UpdateProfile(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    template_name = 'blog_hillel/registration/update_profile.html'
...
class UserDetailView(generic.ListView):
    model = Blog
    paginate_by = 5
    template_name = 'articles/user_detail.html'
    success_message = 'Profile Updated'
...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        form = EmailBlogForm()
    return render(request, 'articles/post_share.html', {'post': post,
                                                        'form': form,
                                                        'sent': sent})
